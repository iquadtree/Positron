    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat TRON and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002TRON per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizePositronAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fTRONSubmitted = (sessionTotalValue / CENT);
        printf("Submitting Darksend for %f TRON CENT - sessionTotalValue %d\n", fTRONSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100TRON+1 ( bit on if present )
    // bit 1 - 10TRON+1
    // bit 2 - 1TRON+1
    // bit 3 - .1TRON+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool(void* parg)
{
    ...
    RenameThread("Positron-darksend");
        ...
        if(c % (60*5)*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizePositronAmount = (rand() % nLeftToAnon)+3;
                } else {
